<x-filament-panels::page>
            ...
            @foreach ($details as $i => $item)
                {
                    ...
                    available_units: @js($item['available_units'] ?? [])
                },
            @endforeach
        ],
    ...
    ($quoteWarehouse->estatus === 'Atendido' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400' :
        'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400') }}">
            ...
            {{-- Quick action bar --}}
            <div
                class="border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 px-5 py-2.5 flex items-center justify-between">
                ...
                <button type="button"
                    @click="items.forEach(i => { const f = i.solicitado - i.entregado; i.despachar = f > 0 ? f : 0; })"
        ...
        <div
            class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-sm rounded-xl overflow-hidden mb-24">
                ...
                <table class="quote-table w-full relative">
                    <thead class="sticky top-0 z-10 bg-gray-100 dark:bg-gray-800 shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
                        <tr>
                            ...
                            <th class="quote-table__th bg-blue-50 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 border-b-2 border-blue-200 dark:border-blue-800 font-bold"
                                style="width: 100px;">Despachar</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                        @foreach ($details as $i => $item)
                            ...
                            @endphp
                            <tr
                                class="transition-colors hover:bg-gray-50/80 dark:hover:bg-gray-800/50 {{ $completado ? 'bg-gray-50/50 dark:bg-gray-800/20' : '' }}">
                                {{-- Tipo --}}
                                <td
                                    class="px-3 py-2 text-center align-middle border-r border-gray-100 dark:border-gray-800/50">
                                    <span
                                        class="inline-flex px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wider rounded border {{ $typeColor }} opacity-80">
                                        {{ substr($typeName, 0, 3) }}.
                                    </span>
                                </td>
                                ...
                                <td class="px-3 py-2 align-middle border-r border-gray-100 dark:border-gray-800/50">
                                    <div
                                        class="text-[11px] font-semibold text-gray-800 dark:text-gray-200 leading-snug {{ $completado ? 'line-through opacity-50' : '' }}">
                                        {{ $item['product_name'] ?? '-' }}
                                    </div>
                                </td>
                                ...
                                {{-- Unidad --}}
                                <td
                                    class="px-3 py-2 text-center align-middle border-r border-gray-100 dark:border-gray-800/50 text-[10px] text-gray-400 font-medium">
                                    {{ $item['unit_name'] ?? '-' }}
                                </td>
                                ...
                                {{-- Previo (Entregado) --}}
                                <td
                                    class="px-3 py-2 text-center align-middle border-r border-gray-100 dark:border-gray-800/50 group">
                                    <div class="flex flex-col items-center gap-1 relative">
                                        ...
                                        <div class="w-10 h-1 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                                            <div class="h-full {{ $completado ? 'bg-emerald-500' : 'bg-gray-400' }} rounded-full"
                                ...
                                <td
                                    class="px-2 py-1.5 align-middle bg-blue-50/30 dark:bg-blue-900/10 border-r border-blue-100 dark:border-blue-900/30">
                                    @if ($completado)
                                        <div
                                            class="flex items-center justify-center gap-1 text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 py-1.5 rounded border border-emerald-100 dark:border-emerald-800/50">
                                        ...
                                        <select x-model="items[{{ $i }}].location_origin_id"
                                            ...
                                            <template x-for="loc in locations" :key="loc.id">
                                                <option :value="loc.id" x-text="loc.name"
                                                    :selected="loc.id == items[{{ $i }}].location_origin_id"></option>
                                            </template>
                                        </select>
                                        @if (!$completado)
                                            <button type="button" @click="openNewLocationModal('origin', {{ $i }})"
                                                class="text-gray-400 hover:text-blue-600 bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/30 border border-gray-200 dark:border-gray-700 rounded p-0.5 transition"
                                                title="Nuevo lugar">
                                ...
                                <td class="px-2 py-1.5 align-middle border-r border-gray-100 dark:border-gray-800/50">
                                    <div class="flex items-center gap-1">
                                        <select x-model="items[{{ $i }}].location_destination_id"
                                            ...
                                            <template x-for="loc in locations" :key="loc.id">
                                                <option :value="loc.id" x-text="loc.name"
                                                    :selected="loc.id == items[{{ $i }}].location_destination_id"></option>
                                            </template>
                                        </select>
                                        @if (!$completado)
                                ...
                                {{-- Comentarios / Serie / Tool Unit Selector --}}
                                <td class="px-2 py-1.5 align-middle">
                                    <template x-if="items[{{ $i }}].is_tool">
                                        <div class="mb-1">
                                            <select x-model="items[{{ $i }}].tool_unit_id"
                                                class="w-full text-[11px] font-medium border-gray-200 dark:border-gray-700 bg-amber-50 dark:bg-amber-900/10 focus:border-amber-500 focus:ring-amber-500 rounded py-1 pl-2 pr-6 text-amber-900 dark:text-amber-100 {{ $completado ? 'opacity-50' : '' }}"
                                                :disabled="{{ $completado ? 'true' : 'false' }}">
                                                <option value="">Seleccione Unidad...</option>
                                                <template x-for="unit in items[{{ $i }}].available_units" :key="unit.id">
                                                    <option :value="unit.id"
                                                        x-text="(unit.internal_code || 'S/C') + ' - ' + (unit.serial_number || 'S/S')"
                                                        :selected="unit.id == items[{{ $i }}].tool_unit_id"></option>
                                                </template>
                                            </select>
                                        </div>
                                    </template>
                                ...
                                <td class="px-1 py-1.5 text-center align-middle">
                                    <button type="button"
                                        @click="openHistoryModal({{ $item['project_requirement_id'] }}, '{{ addslashes($item['product_name']) }}')"
                                        class="inline-flex items-center justify-center text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded p-1 transition"
            ...
            <div
                class="bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 px-5 py-3 flex items-center justify-between">
                ...
                <div class="flex items-center gap-6">
                    <div class="flex items-center gap-2">
                        ...
                        <div class="flex items-center gap-2">
                            <div
                                class="w-32 h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden shadow-inner">
        ...
        <div x-show="newLocationModal.open" x-cloak
            ...
            x-transition.opacity duration.200ms @keydown.escape.window="closeNewLocationModal()">
            <div class="w-full max-w-md bg-white rounded-xl shadow-2xl dark:bg-gray-900 border border-gray-200 dark:border-gray-800"
                @click.outside="closeNewLocationModal()" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="p-5 space-y-4 bg-gray-50/50 dark:bg-gray-800/20">
                    <div>
                        <label
                            class="block text-[11px] font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Nombre
                            <span class="text-red-500">*</span></label>
                        <input type="text" x-model="newLocationModal.name" x-ref="newLocationName"
                            @keydown.enter.prevent="createLocation()"
                            class="w-full text-sm border-gray-300 dark:border-gray-600 focus:border-blue-500 focus:ring-blue-500 rounded-lg shadow-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white py-2 px-3"
                            placeholder="Ej: Almacén Principal">
                    </div>
                    <div>
                        <label
                            class="block text-[11px] font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Descripción
                            <span class="font-normal text-gray-400">(Opcional)</span></label>
                ...
                <div
                    class="px-5 py-3 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 flex justify-end gap-3 rounded-b-xl">
                    <button type="button" @click="closeNewLocationModal()"
                        class="px-4 py-2 text-xs font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition">
                        Cancelar
                    </button>
                    <button type="button" @click="createLocation()"
                        ...
                        <span x-show="!newLocationModal.loading"
                            class="material-symbols-outlined text-[16px]">save</span>
        ...
        <div x-show="historyModal.open" x-cloak
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm"
            x-transition.opacity duration.200ms @keydown.escape.window="closeHistoryModal()">
            <div class="w-full max-w-lg bg-white rounded-xl shadow-2xl dark:bg-gray-900 border border-gray-200 dark:border-gray-800 flex flex-col max-h-[90vh]"
                @click.outside="closeHistoryModal()" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-gray-500 dark:text-gray-400 font-medium truncate max-w-[250px]"
                                x-text="historyModal.productName"></p>
                        </div>
                    </div>
                ...
                <div class="p-5 overflow-y-auto custom-scrollbar bg-gray-50/50 dark:bg-gray-800/20 flex-1 relative">
                    ...
                    <div x-show="historyModal.loading"
                        class="absolute inset-0 bg-gray-50/80 dark:bg-gray-900/80 flex flex-col items-center justify-center z-10 backdrop-blur-sm">
                    ...
                    {{-- Empty State --}}
                    <div x-show="!historyModal.loading && historyModal.transactions.length === 0"
                        class="text-center py-8">
                        <span
                            class="material-symbols-outlined text-4xl text-gray-300 dark:text-gray-600 mb-2">inbox</span>
                        <p class="text-sm text-gray-500 font-medium">No hay despachos registrados para este ítem.</p>
                    </div>

                    {{-- Timeline --}}
                    <div x-show="!historyModal.loading && historyModal.transactions.length > 0"
                        class="relative pl-4 space-y-6 before:absolute before:inset-0 before:ml-[23px] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-200 dark:before:via-gray-700 before:to-transparent">
                        <template x-for="(tx, index) in historyModal.transactions" :key="index">
                            <div class="relative flex items-start gap-4">
                                {{-- Timeline Point --}}
                                <div
                                    class="absolute left-[-1.3rem] mt-1.5 w-3 h-3 rounded-full bg-blue-500 ring-4 ring-white dark:ring-gray-900 shrink-0 shadow-sm z-10 text-[0px]">
                                    .</div>

                                {{-- Content Card --}}
                                <div
                                    class="flex-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 shadow-sm hover:border-blue-300 dark:hover:border-blue-700 transition-colors">
                                    <div class="flex items-center justify-between mb-1">
                                        <div class="flex items-center gap-1.5">
                                            <span
                                                class="material-symbols-outlined text-[14px] text-gray-400">person</span>
                                            <span class="text-xs font-bold text-gray-700 dark:text-gray-300"
                                                x-text="tx.employee"></span>
                                        </div>
                                    ...
                                    <div
                                        class="my-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-100 dark:border-blue-900/50 flex items-center gap-2">
                                        <span
                                    ...
                                    <div class="grid grid-cols-2 gap-2 mt-2">
                                        <div class="text-[10px] text-gray-500 flex items-start gap-1">
                                            <span
                                                class="material-symbols-outlined text-[12px] text-gray-400 shrink-0 mt-0.5">location_on</span>
                                            <div>
                                                <span
                                                    class="block text-gray-400 uppercase tracking-widest text-[8px] font-bold">Desde</span>
                                                <span class="font-medium text-gray-700 dark:text-gray-300"
                                                    x-text="tx.origin"></span>
                                            </div>
                                        </div>
                                        <div
                                            class="text-[10px] text-gray-500 flex items-start gap-1 border-l border-gray-100 dark:border-gray-700 pl-2">
                                            <span
                                                class="material-symbols-outlined text-[12px] text-gray-400 shrink-0 mt-0.5">flag</span>
                                            <div>
                                                <span
                                                    class="block text-gray-400 uppercase tracking-widest text-[8px] font-bold">Hacia</span>
                                                <span class="font-medium text-gray-700 dark:text-gray-300"
                                                    x-text="tx.destination"></span>
                                            </div>
                                        </div>
                                    </div>

                                    <div x-show="tx.comment !== '-'"
                                        class="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700 text-[10px] flex gap-1.5 items-start">
                                        <span
                                            class="material-symbols-outlined text-[12px] text-amber-500 shrink-0">edit_note</span>
                                        <span class="text-gray-600 dark:text-gray-400 italic"
                                            x-text="tx.comment"></span>
                                    </div>

                                    <div x-show="tx.tool_unit"
                                        class="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700 text-[10px] flex gap-1.5 items-center">
                                        <span
                                            class="material-symbols-outlined text-[14px] text-emerald-500 shrink-0">build</span>
                ...
                {{-- Modal Footer --}}
                <div
                    class="px-5 py-3 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 flex justify-end shrink-0 rounded-b-xl">
                    <button type="button" @click="closeHistoryModal()"
                        class="px-4 py-2 text-xs font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition">
                        Cerrar
                    </button>
