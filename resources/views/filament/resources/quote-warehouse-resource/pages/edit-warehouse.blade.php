        ...
        <div class="mt-6">
            <div class="flex items-center gap-3 mb-4">
                <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-emerald-100 dark:bg-emerald-900/30">
                    <span class="material-symbols-outlined text-emerald-600 dark:text-emerald-400 text-[22px]">inventory_2</span>
                </div>
                <div>
                    <h2 class="text-lg font-bold tracking-tight text-slate-900 dark:text-white">Suministros</h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400">Control de materiales y suministros del proyecto</p>
                </div>
            </div>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            @foreach ($details as $i => $item)
                                ...
                                <tr class="transition-colors group hover:bg-slate-50 dark:hover:bg-slate-800/50 {{ $completado ? 'bg-slate-50/50 dark:bg-slate-800/30' : '' }}">
                                    <td :class="items[{{ $i }}].entregado + items[{{ $i }}].despachar >= items[{{ $i }}].solicitado ? 'font-mono text-xs text-center align-middle text-slate-400 dark:text-slate-500 line-through' : 'font-mono text-xs text-center align-middle text-slate-900 dark:text-white'">
                                        {{ $item['sat_line'] ?? '-' }}
                                    </td>
                                    ...
                                    <td class="px-4 py-4 align-top">
                                        <input type="text" x-model="items[{{ $i }}].comment"
                                            class="block w-full rounded-md border-0 py-1.5 text-slate-900 ring-1 ring-inset ring-slate-300 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-primary-600 text-xs dark:bg-slate-900 dark:ring-slate-600 dark:text-white"
